...
\title{IACPG: Interrupt-Aware Code Property Graph for Agent-Based Defect Detection in Embedded Interrupt-Driven Programs} %TODO Please add
\author{Anonymous Author(s)}{Anonymous Affiliation}{anonymous@example.edu}{}{}

\authorrunning{Anonymous Authors}

\Copyright{Anonymous Author(s)} 

% 如果要求完全匿名，建议将 ccsdesc 注释掉或替换为通用占位符
% \ccsdesc[100]{\textcolor{red}{Replace ccsdesc macro with valid one}} 
\ccsdesc[100]{General and reference~Surveys and overviews} % 可选：使用极通用的分类

% 关键词中删除可能指向特定项目或团队的术语（如 “Defect detection” 如果是你团队独有名称，可改为更泛的词）
\keywords{Software analysis, Formal methods, Code representation, Embedded systems}
...
% \authorrunning{J. Open Access and J.\,R. Public} %TODO 
...
% \keywords{Defect detection, Agent-based analysis, Code Property Graph, Interrupt semantics, Embedded systems } %TODO mandatory; please add comma-separated list of keywords
...
\EventNoEds{2}
\EventLongTitle{42nd Conference on Very Important Topics (ESEM 2026)}
\EventShortTitle{ESEM 2026}
\EventAcronym{ESEM}
\EventYear{2026}
\EventDate{December 24--27, 2026}
...
\begin{abstract}
Interrupt-driven programs are pervasive in safety-critical embedded systems, yet their asynchronous preemption, priority nesting, and masking semantics are hard to express in standard program representations, leaving agent-based defect detection without the structured evidence needed for accurate reasoning. This paper proposes the Interrupt-Aware Code Property Graph (IACPG), which overlays interrupt roles, preemption relationships, guard constraints, and cross-context shared access edges onto standard CPG; a hybrid extraction stage combines a rule-based knowledge base with restricted LLM completion to recover ISR identity, priorities, masking scopes, and shared accesses for the target architectures evaluated in this work, and a controlled rule-holdout stress test further probes the completion path beyond deterministic rule coverage.
Evaluated on 96 cross-platform instances spanning four defect patterns and four ISAs, Agent + IACPG achieves 95.6\% precision, 97.7\% recall, and 96.6\% F1, outperforming Agent (CPG) by 8.0pp and LLM Zero-shot by 6.8pp in F1. Compared to standard CPG, IACPG reduces context length by 88.7\% and end-to-end response time by 88.4\%, while improving evidence completeness from 6.2\% to 86.5\%.
\end{abstract}
...
% 第三版（太短了）
% Interrupt-driven programs are widely used in embedded scenarios such as industrial control, automotive electronics, aerospace, and smart terminals, and their reliability directly impacts physical devices and critical tasks \cite{martinez2014hardware, pan2019easy, brylow2004deadline}. Unlike sequential programs, their behavior is not determined by explicit control flow: asynchronous ISR triggering, priority nesting, and interrupt enable/mask scopes jointly shape the true execution order and shared-state access semantics.

% These semantics pose fundamental challenges to code analysis. Interrupt concurrency defects are often concealed and difficult to reproduce \cite{li2023empirical}, yet can cause data inconsistency, system crashes, or loss of control over physical devices. Despite substantial progress in program graph modeling \cite{yamaguchi2014modeling, mi2023graph, liu2023learning, shezan2023chkplug}, defect detection \cite{eslamimehr2018efficient, zhang2025effectively, qiu2024vulnerability, smaili2025transformer, ma2025value, wang2025sift}, and embedded concurrency analysis \cite{wang2017automatic, li2022precise, yu2023detecting, wang2022specchecker, wu2016static, zhang2025bounded, zhao2025formal, wu2015numerical, schwarz2011static, shi2015verifying, yu2018conpredictor, wang2020automatic, liang2017effective, ma2026integrating}, interrupt-driven scenarios remain underserved. Mainstream representations such as AST, CFG, PDG, and CPG still lack direct mechanisms for encoding interrupt semantics \cite{zhu2023code}, while many embedded analyzers rely on architecture-specific rules or specialized logic that limit transferability and unified evidence construction \cite{feng2020rchecker, wang2025specchecker}.

% LLM-driven code agents have shown strong potential in program comprehension, defect localization, automated auditing, and code review \cite{li2024enhancing, liu2025llm, wang2024llmdfa, nguyen2025empirical, li2025codedoctor, yu2024fine, nashaat2024towards}. Their effectiveness in interrupt-driven programs, however, still depends on whether the underlying representation can expose interrupt-specific semantics in a structured form \cite{lekssays2025llmxcpg, shao2025graphfvd, peng2025keep}; without explicit modeling of ISR identity, preemption relationships, masking scope, and cross-context shared access, agents must reason over noisy evidence and cannot easily build interpretable defect arguments.

% Based on these observations, we argue that the key to agent-based analysis of interrupt-driven programs is a unified representation that explicitly expresses interrupt execution semantics. Building on standard CPG, such a representation should carry ISR identity, interrupt switches, priorities, and shared accesses as explicit semantic facts, and derive interrupt roles, preemption relationships, switch constraints, and cross-context shared access relationships as structured evidence amenable to agent querying. To this end, we propose \iacpg for agent-based analysis of embedded interrupt-driven programs.


The main contributions are as follows:
\begin{itemize}
  \item A rule-based interrupt semantic extraction method with CPG structural validation, plus a restricted LLM completion path as a controlled supplement for out-of-coverage scenarios, further probed by a controlled rule-holdout stress test.
  \item \iacpg, a non-destructive semantic overlay on standard CPG that encodes interrupt roles, preemption, guard, and cross-context shared-access relationships as an agent-friendly representation.
  \item A cross-platform evaluation covering 4 defect patterns and 4 ISAs (96 instances), demonstrating significant improvements in defect detection accuracy, false positive control, and diagnostic evidence completeness.
\end{itemize}
...
Formally, let the embedded project be $P$, and the standard code property graph be
\begin{equation}
...
Thus, \iacpg is defined as the result of the standard CPG mapped through the semantic enhancement mapping $\Phi$:
\begin{equation}
...
\begin{algorithmic}[1]
\Require Embedded C project source $P$, standard CPG extractor $\mathcal{B}$, architecture \& rule knowledge base $\mathcal{K}$, Agent $\mathcal{A}$
...
\begin{itemize}
  ...
  \item \textbf{RQ2:} Compared to standard CPG, does the explicit modeling of interrupt semantics in \iacpg significantly reduce the query load and reasoning hops for agents during defect mining?
...
\begin{table}[!t]
  ...
  \end{tabular}}
  \\[4pt]
  \footnotesize{simple\_001--004: public benchmark seeds; simple\_005--006: manually curated, architecture-adapted, and LLM-assisted extensions derived from real-world code patterns.}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{RQ2: Standard CPG vs.\ \iacpg in query efficacy and evidence quality.}
  \label{tab:rq2}
  \resizebox{\columnwidth}{!}{
  \begin{tabular}{lccc}
    \toprule
    \textbf{Evaluation Metric} & \textbf{Standard CPG} & \textbf{IACPG}& \textbf{Improvement} \\
    \midrule
...
\begin{rqbox}
\textbf{Answer to RQ2:}
IACPG substantially improves the combined representation-and-retrieval workflow: compared with standard CPG, it shortens context, reduces reasoning hops, and improves interrupt-evidence quality. These gains indicate that interrupt-aware semantic edges allow the agent to reach defect-relevant evidence with much less search noise.
\end{rqbox}

% \noindent\textbf{Answer to RQ2:} IACPG substantially improves the combined representation-and-retrieval workflow: compared with standard CPG, it shortens context, reduces reasoning hops, and improves interrupt-evidence quality. These gains indicate that interrupt-aware semantic edges allow the agent to reach defect-relevant evidence with much less search noise.
...
% \noindent\textbf{Answer to RQ3:} Agent + IACPG delivers the strongest end-to-end defect detection performance across all four ISAs, achieving 95.6\% precision, 97.7\% recall, and 96.6\% F1 overall. It improves over both Agent (CPG) and LLM Zero-shot while keeping false positives limited to 4 of the 8 negative-control instances and yielding the largest per-ISA gain on MSP430.
...
We propose IACPG, an interrupt-semantics-enhanced code property graph for agent-based defect detection of embedded interrupt-driven programs. By combining a restricted rule base with structured evidence verification, IACPG systematically recovers ISR entities, execution priority topologies, guard masking logic, and cross-context shared-state interactions, then injects these facts as a multi-dimensional semantic overlay atop a standard CPG. In an end-to-end evaluation across 4 ISAs and 96 instances, Agent\,+\,IACPG achieves 95.6\% precision, 97.7\% recall, and 96.6\% F1—outperforming Agent (CPG) at 88.6\% and LLM Zero-shot at 89.8\%—while reducing context length by 88.7\%, end-to-end response time by 88.4\%, and improving evidence completeness from 6.2\% to 86.5\% and SNR from 2.56\% to 19.57\%. Under the current deterministic rule base, static extraction remains near-perfect within configured rule coverage, and a controlled rule-holdout stress test further shows that the restricted completion path can substantially recover held-out ISR conventions across the four target ISAs.

Future work will extend the rule base to multi-core heterogeneous processors and RTOS environments, integrate lightweight path-sensitive alias analysis to tighten false-positive bounds on interrupt protection scopes, and connect IACPG's structured semantic foundation to SMT solvers for fully automated patch-level verification.
